...
export function HouseCard({ house, className = '' }: HouseCardProps) {
  ...
  const getHouseBackground = (houseName: string | null) => {
    ...
    if (name?.includes('gryffindor')) return '/GryffindorCard.svg';
    if (name?.includes('slytherin')) return '/SlytherinCard.svg';
    if (name?.includes('hufflepuff')) return '/HufflepuffCard.svg';
    if (name?.includes('ravenclaw')) return '/RavenclawCard.svg';
    return null;
  };
